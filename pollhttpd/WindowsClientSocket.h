...
#ifdef _WIN32
#ifndef SRC_WINDOWSCLIENTSOCKET_H_
#define SRC_WINDOWSCLIENTSOCKET_H_
...
namespace dnszaikin::pollhttpd::network {
	...
	public:
		WindowsClientSocket() : AbstractSocket(), _send_count(0) {
		...
		*/
		...
		void init(std::string_view host, std::string_view port, socket_t socket_fd, IHandlerFactory::ptr handler) override {
			AbstractSocket::init(host, port, socket_fd, handler);

			u_long opt = 1;
			ioctlsocket(get_socket(), FIONBIO, &opt);
			set_connected();
		}
		...
		callback begin_send() override {
			...
			if (is_connected() && _send_buffer.size() > 0) {
				...
				size = ::send(get_socket(), send_buffer.data(), send_buffer.size(), 0); //SIGPIPE on WIN32 is not generated, so we don't need MSG_NOSIGNAL
			...
			return std::bind(&WindowsClientSocket::end_send, this, complete, size);
		}
		...
		callback begin_recv() override {
			...
			if (is_connected()) {
				...
				size = ::recv(get_socket(), (char*)&buffer, READ_BUF_SIZE, 0);
			...
			return std::bind(&WindowsClientSocket::end_recv, this, complete, size);
		}
...
#endif /* SRC_WINDOWSCLIENTSOCKET_H_ */
#endif
